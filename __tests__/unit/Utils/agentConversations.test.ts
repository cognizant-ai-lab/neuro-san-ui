...
describe("agentConversations", () => {
    ...
    describe("processChatChunk", () => {
        ...
        it("should remove frontman from all conversations when it ends", () => {
            // Setup: Create multiple conversations with frontman in each
            const conversations = [createConversation(), createConversation(), createConversation()]

            // Add frontman to all conversations, plus other agents
            conversations[0].agents.add("frontman")
            conversations[0].agents.add("agent1")
            conversations[1].agents.add("frontman")
            conversations[1].agents.add("agent2")
            conversations[2].agents.add("frontman")
            conversations[2].agents.add("agent3")

            currentConversations = conversations

            // Mock frontman end event
            mockChatMessageFromChunk.mockReturnValue({
                type: ChatMessageType.AGENT,
            ...
            // Verify frontman was removed from all conversations
            const finalConversations = setCurrentConversations.mock.calls[0][0]
            expect(finalConversations).toHaveLength(3)

            // All conversations should still exist but without frontman
            expect(finalConversations[0].agents.has("frontman")).toBe(false)
            expect(finalConversations[0].agents.has("agent1")).toBe(true)
            expect(finalConversations[1].agents.has("frontman")).toBe(false)
            expect(finalConversations[1].agents.has("agent2")).toBe(true)
            expect(finalConversations[2].agents.has("frontman")).toBe(false)
            expect(finalConversations[2].agents.has("agent3")).toBe(true)
        })

        it("should remove empty conversations when frontman was the only agent", () => {
            // Setup: Create conversation with only frontman
            const conversations = [createConversation(), createConversation()]

            conversations[0].agents.add("frontman")
            conversations[1].agents.add("frontman")
            conversations[1].agents.add("agent1")
            ...
            // Verify conversation with only frontman was removed
            const finalConversations = setCurrentConversations.mock.calls[0][0]
